<?php
	...
	$data = mysqli_query($conn,"SELECT *, sum(b.quantity) as qty FROM tbl_sales_order a INNER JOIN tbl_sales_order_detail b WHERE a.sales_order_id = b.sales_order_id AND a.status = 1 AND a.date_added BETWEEN '$from' AND '$to' GROUP BY a.sales_order_id ORDER BY a.sales_order_id ASC");
	$response["data"] = array();
	$total_amount = 0;
	$total_qty = 0;
	while($row = mysqli_fetch_array($data)){
		$total_qty += $row["qty"];
		$total_amount += get_detail_amount($row["sales_order_id"], $conn);
		...
		$list["amount"] = get_detail_amount($row["sales_order_id"], $conn);
