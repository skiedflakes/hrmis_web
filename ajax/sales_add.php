<?php
	...
	if($check_balance_qty != 0){
		...
		if($sales_id == 0){
			...
			if($add){
				$add_detail = mysqli_query($conn,"INSERT INTO tbl_sales_order_detail SET quantity = quantity + 1, sales_order_id = '$new_sales_id', product_id = '$products', status = 0, date_added = '$date_added'") or die(mysqli_error($conn));
				if($add_detail){
					echo $new_sales_id;
				}else{
					echo 0;
			...
			if($check == 0){
				$add_detail = mysqli_query($conn,"INSERT INTO tbl_sales_order_detail SET quantity = quantity + 1, sales_order_id = '$sales_id', product_id = '$products', status = 0, date_added = '$date_added'") or die(mysqli_error($conn));
				if($add_detail){
					echo $sales_id;
				}else{
					echo 0;
